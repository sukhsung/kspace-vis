classdef BLG < graphene        
    ...
    methods
        ...
        function [pos, mag] = calculateABC(self)
            ...
            for idx = 1:length(self.stacking)
               layer = self.stacking(idx);
               zfactor = exp(-1i.*self.lambda.*kz.*idx);
               if layer == 'A'
                   mag = mag + a0.*zfactor;
               elseif layer=='B'
                   mag = mag+ b0.*zfactor;
               elseif layer == 'C'
                   ...
                   display(['error at index' num2str(idx)]);
               end
            end
            
            mag = s_g.*mag.*exp(1i.*self.lambda.*kz.*(length(self.stacking)+1)./2);
          %  mag = mag.*exp(-1i.*self.lambda.*kz.*(length(self.stacking))/2);
        end
